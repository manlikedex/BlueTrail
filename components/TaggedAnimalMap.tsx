...
function getColor(group: string) {
  ...
  if (value.includes("whale")) return "bg-cyan-400";
...
export default function TaggedAnimalMap({
  ...
  return (
    <div className="relative mt-6 h-[520px] overflow-hidden rounded-3xl border border-[#1A2330] bg-[#05070A]">
      ...
      <div className="absolute right-5 top-5 z-20 rounded-full border border-[#1A2330] bg-[#05070A]/80 px-3 py-2 text-[10px] font-black uppercase tracking-[0.2em] text-[#7D8896]">
        UK Marine Sightings
      </div>
      ...
      {animals.map((animal) => {
        ...
        return (
          ...
          >
            <div
              className={`absolute left-1/2 top-1/2 h-8 w-8 -translate-x-1/2 -translate-y-1/2 animate-ping rounded-full opacity-25 ${getColor(
            ...
            <div className="pointer-events-none absolute left-1/2 top-6 z-30 hidden min-w-[220px] -translate-x-1/2 rounded-2xl border border-[#1A2330] bg-[#05070A]/95 p-3 shadow-2xl backdrop-blur-xl group-hover:block">
              <p className="font-black text-white">{animal.common_name}</p>
      ...
      {animals.length === 0 && (
        <div className="absolute inset-0 z-20 flex items-center justify-center">
          <div className="rounded-2xl border border-[#1A2330] bg-[#05070A]/90 p-6 text-center backdrop-blur-xl">
            <p className="text-lg font-black text-white">No sightings loaded</p>
